...
public class AutoCompleteDictionaryTrie implements Dictionary, AutoComplete {
	...
	public List<String> predictCompletions(String prefix, int numCompletions) {
		...
		return BFS(stem, numCompletions);
	...
	private List<String> BFS(TrieNode t, int numCompletions) {
		...
		while (!queue.isEmpty()) {
			TrieNode x = queue.poll();
			if (x.endsWord()) {
				list.add(x.getText());
			}
			for (char c : x.getValidNextCharacters()) {
				queue.add(x.getChild(c));
			}
			if(numCompletions<=list.size()){
				return list;
			}
		}
		return list;
